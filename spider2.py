...
try:
    with open("contest.txt",mode="r",encoding="utf-8") as f:
        token=f.readline().strip()
        contest=f.readline().strip()
        url=f.readline().strip()
        print(f"THIS IS CONTEST VERSION!!!!\nParameters: \ntoken={token}\ncontest={contest}\nurl={url}\nPlease confirm by pressing Enter")
        confirm()
except:
    print("Cannot read contest.txt! Place your ID on the first line, contest id on the second and base url on the third")
    exit(1)
...
def addScore(prob):
    ...
    while True:
        ...
        while index+2<len(tot):
            ...
            appendScore(tot[index],tot[index+1],parseScore(tot[index+2]),tot[index+2])
            if tot[index+6]=='/':
                index+=12
            else:
                index+=10
...
probs='ABCDEFGHIJKLMNOPQRSTUVWXYZ'
...
with open("out_contest.html",mode="w",encoding="utf-8") as f:
    f.write(out)
